...
test("lead board keeps all 16 Monday columns including empty Tender Received", () => {
  ...
  const leadDeskMain = css.match(/\.desk-main:has\(\.lead-board-shell\)\s*\{[^}]*\}/)?.[0] ?? "";
  const leadSlot = css.match(/\.desk-main:has\(\.lead-board-shell\) \.desk-main-slot,[\s\S]*?\{[^}]*\}/)?.[0] ?? "";
  const leadDeskShell = css.match(/\.desk-shell:has\(\.lead-board-shell\),[\s\S]*?\{[^}]*\}/)?.[0] ?? "";
  const leadPage = css.match(/\.lead-page\s*\{[^}]*\}/)?.[0] ?? "";
  const leadBoardFill = css.match(/\.lead-board-fill\s*\{[^}]*\}/)?.[0] ?? "";
  const leadBoardWrap = css.match(/\.lead-board-wrap\s*\{[^}]*\}/)?.[0] ?? "";
  const leadBoardShell = css.match(/\.lead-board-shell\s*\{[^}]*\}/)?.[0] ?? "";
  assert.match(leadDeskMain, /overflow-x:\s*hidden/);
  assert.match(leadDeskMain, /overflow-y:\s*visible/);
  assert.equal(leadDeskMain.includes("overflow-y: hidden"), false);
  assert.equal(/\.desk-main:has\(\.lead-board-shell\)[^}]*overflow-y:\s*hidden/.test(css), false);
  assert.match(leadSlot, /height:\s*auto/);
  assert.match(leadSlot, /max-height:\s*none/);
  assert.match(leadSlot, /overflow:\s*visible/);
  ...
  assert.match(leadBoard, /height:\s*auto/);
  ...
  assert.equal(leadColumn.includes("max-height"), false);
  ...
  for (const rule of [leadPage, leadBoardFill, leadBoardWrap, leadBoardShell, leadBoard]) {
    assert.match(rule, /height:\s*auto/);
    assert.match(rule, /flex:\s*0 0 auto/);
    assert.equal(rule.includes("flex: 1 1 0"), false);
    assert.equal(rule.includes("height: 100%"), false);
    assert.equal(rule.includes("max-height: 100%"), false);
  }
  assert.match(css, /\.desk-main-slot\s*\{[^}]*flex:\s*1/s);
  assert.match(css, /html:has\(\.lead-board-shell\),\s*html:has\(\.lead-board-shell\) body\s*\{[^}]*overflow-y:\s*auto/s);
